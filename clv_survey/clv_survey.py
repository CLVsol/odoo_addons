...
class clv_survey(osv.osv):
    ...
    _columns = {
        ...
        'page_ids': fields.one2many('clv_survey.page', 'clv_survey_id', 'Page'),
        ...
        'history': fields.one2many('clv_survey.history', 'clv_survey_id', 'History Lines', readonly=True),
    ...
    def action_print_clv_survey(self, cr, uid, ids, context=None):
        ...
        if 'response_id' in context:
            ...
            datas['ids'] = [context.get('clv_survey_id', 0)]
        else:
            response_id = self.pool.get('clv_survey.response').search(cr, uid, [('clv_survey_id','=', ids)], context=context)
            datas['ids'] = ids
        ...
        if response_id and response_id[0]:
            context.update({'clv_survey_id': datas['ids']})
    ...
    def fill_clv_survey(self, cr, uid, ids, context=None):
        ...
        for sur in sur_obj:
            ...
            context.update({'active':False,'clv_survey_id': ids[0]})
        return {
            'view_type': 'form',
            'view_mode': 'form',
    ...
    def edit_clv_survey(self, cr, uid, ids, context=None):
        ...
        for sur in sur_obj:
            ...
            if not pages:
                raise osv.except_osv(_('Warning!'), _('This survey has no question defined. Please define the questions and answers first.'))
            context.update({'clv_survey_id': ids[0]})
        return {
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'clv_survey.question.wiz',
...
class clv_survey_history(osv.osv):
    ...
    _rec_name = 'date'
    _columns = {
        'clv_survey_id': fields.many2one('clv_survey', 'Survey'),
...
class clv_survey_page(osv.osv):
    ...
    _columns = {
        ...
        'clv_survey_id': fields.many2one('clv_survey', 'Survey', ondelete='cascade'),
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        if context.has_key('clv_survey_id'):
            data['clv_survey_id'] = context.get('clv_survey_id', False)
        return data
...
class clv_survey_question(osv.osv):
    ...
    _columns = {
        ...
        'clv_survey': fields.related('page_id', 'clv_survey_id', type='many2one', relation='clv_survey', string='Survey'),
...
class clv_survey_response(osv.osv):
    ...
    _columns = {
        'clv_survey_id' : fields.many2one('clv_survey', 'Survey', required=1, ondelete='cascade'),
...
class res_users(osv.osv):
    ...
    _name = "res.users"
    _columns = {
        'clv_survey_id': fields.many2many('clv_survey', 'clv_survey_users_rel', 'uid', 'sid', 'Groups'),
    }
