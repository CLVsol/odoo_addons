...
class clv_survey_question_wiz(osv.osv_memory):
        ...
        if view_type in ['form']:
            ...
            if sur_name_read.transfer or not sur_name_rec.page_no + 1:
                ...
                if sur_name_read.page == "next" or sur_name_rec.page_no == -1:
                    if total_pages > sur_name_rec.page_no + 1:
                        if ((context.has_key('active') and not context.get('active', False)) \
                            ...
                            res = cr.fetchone()[0]
                            user_limit = clv_survey_obj.browse(cr, uid, survey_id)
                        ...
                        if sur_rec.max_response_limit and sur_rec.max_response_limit <= sur_rec.tot_start_clv_survey and not sur_name_rec.page_no + 1:
                            clv_survey_obj.write(cr, uid, survey_id, {'state':'close', 'date_close':strftime("%Y-%m-%d %H:%M:%S")})
                ...
                if flag:
                    ...
                    clv_survey_obj.write(cr, uid, survey_id, {'tot_comp_clv_survey' : sur_rec.tot_comp_clv_survey + 1})
                    ...
                    if sur_rec.send_response:
                        clv_survey_data = clv_survey_obj.browse(cr, uid, survey_id)
                        response_id = surv_name_wiz.read(cr, uid, context.get('sur_name_id',False))['response']
                        report = self.create_report(cr, uid, [survey_id], 'report.clv_survey.browse.response', clv_survey_data.title,context)
                        attachments = {}
                        pdf_filename = addons.get_module_resource('clv_survey', 'report') + clv_survey_data.title + ".pdf"
                        if os.path.exists(pdf_filename):
                            ...
                            file.close()
                            os.remove(addons.get_module_resource('clv_survey', 'report') + clv_survey_data.title + ".pdf")
    ...
    def default_get(self, cr, uid, fields_list, context=None):
        ...
        for field in fields_list:
            if field.split('_')[0] == 'progress':
                tot_page_id = self.pool.get('clv_survey').browse(cr, uid, context.get('survey_id',False))
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if not safe_eval(sur_name_read['store_ans']):
            ...
            sur_rec = clv_survey_obj.read(cr, uid, survey_id)
            clv_survey_obj.write(cr, uid, survey_id,  {'tot_start_clv_survey' : sur_rec['tot_start_clv_survey'] + 1})
    ...
    def action_delete_page(self, cr, uid, ids, context=None):
        ...
        for clv_survey in self.pool.get('clv_survey').browse(cr, uid, [context.get('survey_id',False)], context=context):
