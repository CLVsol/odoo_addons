...
class hr_employee(models.Model):
    ...
    code = fields.Char('Employee Code', size=64, select=1, required=False, readonly=False, default='/',
                       help='Use "/" to get an automatic new Employee Code.')

    @api.model
    def create(self, vals):
        if 'code' not in vals or ('code' in vals and vals['code'] == '/'):
            code_seq = self.pool.get('ir.sequence').get(self._cr, self._uid, 'hr.employee.code')
