...
class clv_medicament_manufacturer(osv.osv):
    ...
    _columns = {
        'code': fields.char('Manufacturer Code', size=64, select=1, required=False, readonly=False, default='/',
                            help='Use "/" to get an automatic new Manufacturer Code.'),
        }
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if not 'code' in vals or ('code' in vals and vals['code'] == '/'):
            code_seq = self.pool.get('ir.sequence').get(cr, uid, 'clv_medicament.manufact.code')
            vals['code'] = format_code(code_seq)
        return super(clv_medicament_manufacturer, self).create(cr, uid, vals, context)
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if ('code' in vals and vals['code'] == '/'):
            code_seq = self.pool.get('ir.sequence').get(cr, uid, 'clv_medicament.manufact.code')
            vals['code'] = format_code(code_seq)
        return super(clv_medicament_manufacturer, self).write(cr, uid, ids, vals, context)

    def copy(self, cr, uid, id, default={}, context=None):
        default = dict(default or {})
        default.update({'code': '/',})
        return super(clv_medicament_manufacturer, self).copy(cr, uid, id, default, context)
