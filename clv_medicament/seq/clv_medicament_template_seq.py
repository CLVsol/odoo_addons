...
class clv_medicament_template(osv.osv):
    ...
    _columns = {
        'name': fields.char('Medicament Template Code', size=256, select=1, required=True, readonly=False, default='/',
                            help='Use "/" to get an automatic new Medicament Template Code.'),
        }
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if not 'name' in vals or ('name' in vals and vals['name'] == '/'):
            code_seq = self.pool.get('ir.sequence').get(cr, uid, 'clv_medicament_template.code')
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if ('name' in vals and vals['name'] == '/'):
            code_seq = self.pool.get('ir.sequence').get(cr, uid, 'clv_medicament_template.code')
            vals['name'] = format_code(code_seq)
        return super(clv_medicament_template, self).write(cr, uid, ids, vals, context)

    def copy(self, cr, uid, id, default={}, context=None):
        default = dict(default or {})
        default.update({'name': '/',})
        return super(clv_medicament_template, self).copy(cr, uid, id, default, context)
