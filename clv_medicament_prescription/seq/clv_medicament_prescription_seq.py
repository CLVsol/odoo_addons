...
class clv_medicament_prescription(osv.osv):
    ...
    _columns = {
        'name': fields.char('Prescription Code', size=256, select=1, required=True, readonly=False, default='/',
                            help='Use "/" to get an automatic new Prescription Code.'),
        }
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if not 'name' in vals or ('name' in vals and vals['name'] == '/'):
            code_seq = self.pool.get('ir.sequence').get(cr, uid, 'clv_medicament_prescription.code')
            vals['name'] = format_code(code_seq)
        return super(clv_medicament_prescription, self).create(cr, uid, vals, context)
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if ('name' in vals and vals['name'] == '/'):
            code_seq = self.pool.get('ir.sequence').get(cr, uid, 'clv_medicament_prescription.code')
            vals['name'] = format_code(code_seq)
        return super(clv_medicament_prescription, self).write(cr, uid, ids, vals, context)

    def copy(self, cr, uid, id, default={}, context=None):
        default = dict(default or {})
        default.update({'name': '/',})
        return super(clv_medicament_prescription, self).copy(cr, uid, id, default, context)
