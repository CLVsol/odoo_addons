...
class clv_pharmacy_category(models.Model):
    _name = 'clv_pharmacy.category'

    name = fields.Char('Category', required=True, size=64, translate=True)
    parent_id = fields.Many2one('clv_pharmacy.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('clv_pharmacy.category', 'parent_id', 'Child Categories')
    active = fields.Boolean('Active', 
                            help="If unchecked, it will allow you to hide the category without removing it.",
                            default=1)
    parent_left = fields.Integer('Left parent', select=True)
    parent_right = fields.Integer('Right parent', select=True)
    pharmacy_ids = fields.Many2many('clv_pharmacy', 
                                    'clv_pharmacy_category_rel', 
                                    'category_id', 
                                    'pharmacy_id', 
                                    'Pharmacies')
...
class clv_pharmacy(models.Model):
    ...
    category_ids = fields.Many2many('clv_pharmacy.category', 
                                    'clv_pharmacy_category_rel', 
                                    'pharmacy_id', 
                                    'category_id', 
                                    'Categories')
