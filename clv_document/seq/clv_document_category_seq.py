...
class clv_document_category(osv.osv):
    _inherit = 'clv_document.category'
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if not 'code' in vals or ('code' in vals and vals['code'] == '/'):
            code_seq = self.pool.get('ir.sequence').get(cr, uid, 'clv_document.category.code')
            vals['code'] = format_code(code_seq)
        return super(clv_document_category, self).create(cr, uid, vals, context)
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if ('code' in vals and vals['code'] == '/'):
            code_seq = self.pool.get('ir.sequence').get(cr, uid, 'clv_document.category.code')
            vals['code'] = format_code(code_seq)
        return super(clv_document_category, self).write(cr, uid, ids, vals, context)

    def copy(self, cr, uid, id, default={}, context=None):
        default = dict(default or {})
        default.update({'code': '/',})
        return super(clv_document_category, self).copy(cr, uid, id, default, context)
