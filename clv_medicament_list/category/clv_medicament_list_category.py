...
class clv_medicament_list_category(models.Model):
    _name = 'clv_medicament_list.category'

    name = fields.Char('Category', required=True, size=64, translate=True)
    parent_id = fields.Many2one('clv_medicament_list.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('clv_medicament_list.category', 'parent_id', 'Child Categories')
    active = fields.Boolean('Active', 
                            help="If unchecked, it will allow you to hide the category without removing it.",
                            default=1)
    parent_left = fields.Integer('Left parent', select=True)
    parent_right = fields.Integer('Right parent', select=True)
    medicament_list_ids = fields.Many2many('clv_medicament_list', 
                                 'clv_medicament_list_category_rel', 
                                 'category_id', 
                                 'medicament_list_id', 
                                 'Medicament Lists')
...
class clv_medicament_list(models.Model):
    _inherit = 'clv_medicament_list'

    category_id = fields.Many2one('clv_medicament_list.category', 'Category')
