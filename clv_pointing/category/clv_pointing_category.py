...
class clv_pointing_category(osv.osv):
    _name = 'clv_pointing.category'
    ...
    _columns = {
        ...
        'parent_id': fields.many2one('clv_pointing.category', 'Parent Category', select=True, ondelete='restrict'),
        ...
        'child_ids': fields.one2many('clv_pointing.category', 'parent_id', 'Child Categories'),
        'active': fields.boolean('Active', 
                                 help="If unchecked, it will allow you to hide the category without removing it."),
        'parent_left': fields.integer('Left parent', select=True),
        'parent_right': fields.integer('Right parent', select=True),
        'pointing_ids': fields.many2many('clv_pointing', 
                                       'clv_pointing_category_rel', 
                                       'category_id', 
                                       'pointing_id', 
                                       'pointings'),
        }
...
class clv_pointing(osv.osv):
    ...
    _columns = {
        'category_ids': fields.many2many('clv_pointing.category', 
                                         'clv_pointing_category_rel', 
                                         'pointing_id', 
                                         'category_id', 
                                         'Categories'),
