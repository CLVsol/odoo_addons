...
class clv_address(models.Model):
    ...
    code = fields.Char('Address Code', size=64, select=1, required=False, readonly=False, default='/',
                       help='Use "/" to get an automatic new Address Code.')
    
    @api.model
    def create(self, vals):
        if not 'code' in vals or ('code' in vals and vals['code'] == '/'):
            code_seq = self.pool.get('ir.sequence').get(self._cr, self._uid, 'clv_address.code')
            vals['code'] = format_code(code_seq)
        return super(clv_address, self).create(vals)
    ...
    def write(self, vals):
        if 'code' in vals and vals['code'] == '/':
            code_seq = self.pool.get('ir.sequence').get(self._cr, self._uid, 'clv_address.code')
            vals['code'] = format_code(code_seq)
        return super(clv_address, self).write(vals)

    @api.one
    def copy(self, default=None):
        default = dict(default or {})
        default.update({'code': '/',})
        return super(clv_address, self).copy(default)
