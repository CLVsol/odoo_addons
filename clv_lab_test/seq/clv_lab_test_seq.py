...
class clv_lab_test(models.Model):
    ...
    def create(self, vals):
        if 'name' not in vals or ('name' in vals and vals['name'] == '/'):
            code_seq = self.pool.get('ir.sequence').get(self._cr, self._uid, 'clv_lab_test.code')
            vals['name'] = format_code(code_seq)
        return super(clv_lab_test, self).create(vals)
    ...
    def write(self, vals):
        if 'name' in vals and vals['name'] == '/':
            code_seq = self.pool.get('ir.sequence').get(self._cr, self._uid, 'clv_lab_test.code')
            vals['name'] = format_code(code_seq)
        return super(clv_lab_test, self).write(vals)

    @api.one
    def copy(self, default=None):
        default = dict(default or {})
        default.update({'name': '/', })
        return super(clv_lab_test, self).copy(default)
