...
class clv_insured_outside_category(models.Model):
    _name = 'clv_insured_outside.category'

    name = fields.Char('Category', required=True, size=64, translate=True)
    parent_id = fields.Many2one('clv_insured_outside.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('clv_insured_outside.category', 'parent_id', 'Child Categories')
    ...
    insured_outside_ids = fields.Many2many('clv_insured_outside', 
                                        'clv_insured_outside_category_rel', 
                                        'category_id', 
                                        'insured_outside_id', 
                                        'Insureds (Outside)')
...
class clv_insured_outside(models.Model):
    ...
    category_ids = fields.Many2many('clv_insured_outside.category', 
                                    'clv_insured_outside_category_rel', 
                                    'insured_outside_id', 
                                    'category_id', 
                                    'Categories')
