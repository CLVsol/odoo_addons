...
class clv_tray_category(models.Model):
    ...
    def create(self, vals):
        if not 'code' in vals or ('code' in vals and vals['code'] == '/'):
            code_seq = self.pool.get('ir.sequence').get(self._cr, self._uid, 'clv_tray.category.code')
            vals['code'] = format_code(code_seq)
        return super(clv_tray_category, self).create(vals)
    ...
    def write(self, vals):
        if 'code' in vals and vals['code'] == '/':
            code_seq = self.pool.get('ir.sequence').get(self._cr, self._uid, 'clv_tray.category.code')
            vals['code'] = format_code(code_seq)
        return super(clv_tray_category, self).write(vals)

    @api.one
    def copy(self, default=None):
        default = dict(default or {})
        default.update({'code': '/',})
        return super(clv_tray_category, self).copy(default)
