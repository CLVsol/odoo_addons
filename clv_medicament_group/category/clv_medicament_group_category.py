...
class clv_medicament_group_category(osv.osv):
    _name = 'clv_medicament_group.category'
    ...
    _columns = {
        ...
        'parent_id': fields.many2one('clv_medicament_group.category', 'Parent Category', select=True, ondelete='restrict'),
        ...
        'child_ids': fields.one2many('clv_medicament_group.category', 'parent_id', 'Child Categories'),
        'active': fields.boolean('Active', 
                                 help="If unchecked, it will allow you to hide the category without removing it."),
        'parent_left': fields.integer('Left parent', select=True),
        'parent_right': fields.integer('Right parent', select=True),
        'medicament_group_ids': fields.many2many('clv_medicament_group', 
                                       'clv_medicament_group_category_rel', 
                                       'category_id', 
                                       'medicament_group_id', 
                                       'medicament_groups'),
        }
...
class clv_medicament_group(osv.osv):
    ...
    _columns = {
        'category_ids': fields.many2many('clv_medicament_group.category', 
                                         'clv_medicament_group_category_rel', 
                                         'medicament_group_id', 
                                         'category_id', 
                                         'Categories'),
