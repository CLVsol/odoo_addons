...
class clv_seedling_category(models.Model):
    _name = 'clv_seedling.category'

    name = fields.Char('Category', required=True, size=64, translate=True)
    parent_id = fields.Many2one('clv_seedling.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('clv_seedling.category', 'parent_id', 'Child Categories')
    active = fields.Boolean('Active', 
                            help="If unchecked, it will allow you to hide the category without removing it.",
                            default=1)
    parent_left = fields.Integer('Left parent', select=True)
    parent_right = fields.Integer('Right parent', select=True)
    seedling_ids = fields.Many2many('clv_seedling', 
                                    'clv_seedling_category_rel', 
                                    'category_id', 
                                    'seedling_id', 
                                    'Seedlings')
...
class clv_seedling(models.Model):
    ...
    category_ids = fields.Many2many('clv_seedling.category', 
                                    'clv_seedling_category_rel', 
                                    'seedling_id', 
                                    'category_id', 
                                    'Categories')
