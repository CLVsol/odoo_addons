...
class clv_place_category(models.Model):
    _name = 'clv_place.category'

    name = fields.Char('Category', required=True, size=64, translate=True)
    parent_id = fields.Many2one('clv_place.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('clv_place.category', 'parent_id', 'Child Categories')
...
class clv_place(models.Model):
    ...
    category_id = fields.Many2one('clv_place.category', 'Category')
