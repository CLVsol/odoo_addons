...
class clv_place_history(models.Model):
    _name = 'clv_place.history'

    place_id = fields.Many2one('clv_place', 'Place', required=True)
...
class clv_place(models.Model):
    ...
    def insert_clv_place_history(self, place_id, state, notes):
        if self.active_history:
            ...
            self.pool.get('clv_place.history').create(self._cr, self._uid, values)

    @api.multi
    def write(self, values):
        if (not 'state' in values) and (not 'date' in values):
            notes = values.keys()
            self.insert_clv_place_history(self.id, self.state, notes)
        return super(clv_place, self).write(values)

    @api.one
    def button_new(self):
        self.date = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        self.state = 'new'
        self.insert_clv_place_history(self.id, 'new', '')
    ...
    def button_activate(self):
        ...
        if not self.date_activation:
            ...
            time.sleep(1.0)
        self.state = 'active'
        self.insert_clv_place_history(self.id, 'active', '')
    ...
    def button_inactivate(self):
        ...
        if not self.date_inactivation:
            ...
            time.sleep(1.0)
        self.state = 'inactive'
        self.insert_clv_place_history(self.id, 'inactive', '')
    ...
    def button_suspend(self):
        ...
        self.insert_clv_place_history(self.id, 'suspended', '')
