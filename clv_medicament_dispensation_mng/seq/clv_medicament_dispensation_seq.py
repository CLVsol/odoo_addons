...
class clv_medicament_dispensation_mng(models.Model):
    _inherit = 'clv_medicament_dispensation_mng'

    name = fields.Char('Dispensation Code', size=64, select=1, required=False, readonly=False, default=False,
                       help='Use "/" to get an automatic new Dispensation Code.')
    
    @api.model
    def create(self, vals):
        if not 'name' in vals or ('name' in vals and vals['name'] == '/'):
            code_seq = self.pool.get('ir.sequence').get(self._cr, self._uid, 'clv_medicament_dispensation.code')
            vals['name'] = format_code(code_seq)
        return super(clv_medicament_dispensation_mng, self).create(vals)
    ...
    def write(self, vals):
        if 'name' in vals and vals['name'] == '/':
            code_seq = self.pool.get('ir.sequence').get(self._cr, self._uid, 'clv_medicament_dispensation.code')
            vals['name'] = format_code(code_seq)
        return super(clv_medicament_dispensation_mng, self).write(vals)

    @api.one
    def copy(self, default=None):
        default = dict(default or {})
        default.update({'name': '/',})
        return super(clv_medicament_dispensation_mng, self).copy(default)
