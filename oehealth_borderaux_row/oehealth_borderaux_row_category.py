...
class oehealth_borderaux_row_category(osv.osv):
    _name = 'oehealth.borderaux.row.category'
    _description = 'Borderaux Row Categories'

    def name_get(self, cr, uid, ids, context=None):
        """Return the category's display name, including their direct
           parent by default.

        :param dict context: the ``borderaux_category_display`` key can be
        ...
        if context is None:
            context = {}
        if context.get('borderaux_category_display') == 'short':
            return super(oehealth_borderaux_row_category, self).name_get(cr, uid, ids, context=context)
    ...
    _columns = {
        ...
        'parent_id': fields.many2one('oehealth.borderaux.row.category', 'Parent Category', select=True, ondelete='cascade'),
        'description': fields.text(string='Description'),
        'info': fields.text(string='Info'),
        'complete_name': fields.function(_name_get_fnc, type="char", string='Category'),
        'child_ids': fields.one2many('oehealth.borderaux.row.category', 'parent_id', 'Child Categories'),
        'active': fields.boolean('Active', help="The active field allows you to hide the category without removing it."),
        'parent_left': fields.integer('Left parent', select=True),
        'parent_right': fields.integer('Right parent', select=True),
        'borderaux_row_ids': fields.many2many('oehealth.borderaux.row', 
...
oehealth_borderaux_row_category()
