...
class clv_frame(models.Model):
    ...
    code = fields.Char(size=64, string='Frame Code', required=False, readonly=False, default='/', 
    	                        help='Use "/" to get an automatic new Code.')

    @api.model
    def create(self, vals):
        if not 'code' in vals or ('code' in vals and vals['code'] == '/'):
            code_seq = self.pool.get('ir.sequence').get(self._cr, self._uid, 'clv_frame.code')
            if 'level' in vals:
                code_level = int(vals['level'][1])
            else:
                code_level = 'l0'
            vals['code'] = format_code(code_seq, code_level)
        return super(clv_frame, self).create(vals)
    ...
    def write(self, vals):
        if 'code' in vals and vals['code'] == '/':
            ...
            if 'level' in vals:
                ...
                code_level = int(vals['level'][1])
                vals['code'] = format_code(code_seq, code_level)
        return super(clv_frame, self).write(vals)

    @api.one
    def copy(self, default=None):
        default = dict(default or {})
        default.update({'code': '/',})
        return super(clv_frame, self).copy(default)
