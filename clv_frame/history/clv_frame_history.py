...
class clv_frame_history(models.Model):
    _name = 'clv_frame.history'

    frame_id = fields.Many2one('clv_frame', 'Frame', required=True)
...
class clv_frame(models.Model):
    ...
    def insert_clv_frame_history(self, frame_id, state, notes):
        if self.active_history:
            ...
            self.pool.get('clv_frame.history').create(self._cr, self._uid, values)

    @api.multi
    def write(self, values):
        if (not 'state' in values) and (not 'date' in values):
            notes = values.keys()
            self.insert_clv_frame_history(self.id, self.state, notes)
        return super(clv_frame, self).write(values)

    @api.one
    def button_new(self):
        self.date = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        self.state = 'new'
        self.insert_clv_frame_history(self.id, 'new', '')
    ...
    def button_activate(self):
        ...
        #     time.sleep(1.0)
        self.state = 'active'
        self.insert_clv_frame_history(self.id, 'active', '')
    ...
    def button_inactivate(self):
        ...
        #     time.sleep(1.0)
        self.state = 'inactive'
        self.insert_clv_frame_history(self.id, 'inactive', '')
    ...
    def button_suspend(self):
        ...
        self.insert_clv_frame_history(self.id, 'suspended', '')
