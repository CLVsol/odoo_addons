...
class clv_frame_category(models.Model):
    _name = 'clv_frame.category'

    name = fields.Char('Category', required=True, size=64, translate=True)
    parent_id = fields.Many2one('clv_frame.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('clv_frame.category', 'parent_id', 'Child Categories')
...
class clv_frame(models.Model):
    ...
    category_id = fields.Many2one('clv_frame.category', 'Category')
