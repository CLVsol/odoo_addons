...
class clv_insured_card_category(models.Model):
    _name = 'clv_insured_card.category'

    name = fields.Char('Category', required=True, size=64, translate=True)
    parent_id = fields.Many2one('clv_insured_card.category', 'Parent Category', select=True, ondelete='restrict')
    ...
    child_ids = fields.One2many('clv_insured_card.category', 'parent_id', 'Child Categories')
    active = fields.Boolean('Active', 
                            help="If unchecked, it will allow you to hide the category without removing it.",
                            default=1)
    parent_left = fields.Integer('Left parent', select=True)
    parent_right = fields.Integer('Right parent', select=True)
    insured_card_ids = fields.Many2many('clv_insured_card', 
                                        'clv_insured_card_category_rel', 
                                        'category_id', 
                                        'insured_card_id', 
                                        'Insureds')
...
class clv_insured_card(models.Model):
    ...
    category_ids = fields.Many2many('clv_insured_card.category', 
                                    'clv_insured_card_category_rel', 
                                    'insured_card_id', 
                                    'category_id', 
                                    'Categories')
