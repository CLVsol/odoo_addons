...
class clv_insured_card(models.Model):
    ...
    code = fields.Char('Insured Card Code', size=64, select=1, required=False, readonly=False, default='/',
                       help='Use "/" to get an automatic new Insured Card Code.')
    
    @api.model
    def create(self, vals):
        if not 'code' in vals or ('code' in vals and vals['code'] == '/'):
            code_seq = self.pool.get('ir.sequence').get(self._cr, self._uid, 'clv_insured_card.code')
            vals['code'] = format_code(code_seq)
        return super(clv_insured_card, self).create(vals)
    ...
    def write(self, vals):
        if 'code' in vals and vals['code'] == '/':
            code_seq = self.pool.get('ir.sequence').get(self._cr, self._uid, 'clv_insured_card.code')
            vals['code'] = format_code(code_seq)
        return super(clv_insured_card, self).write(vals)

    @api.one
    def copy(self, default=None):
        default = dict(default or {})
        default.update({'code': '/',})
        return super(clv_insured_card, self).copy(default)
